...
function getRectangleString(width, height) {
  const top = '┌┐\n'.split('');
  top.splice(1, 0, '─'.repeat(width - 2));
  const bottom = '└┘\n'.split('');
  bottom.splice(1, 0, '─'.repeat(width - 2));
  const middle = '││\n'.split('');
  middle.splice(1, 0, ' '.repeat(width - 2));
  return top.join('') + middle.join('').repeat(height - 2) + bottom.join('');
}
